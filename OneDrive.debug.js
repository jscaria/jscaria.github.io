...
OneDrive.save = function(options) {
    ...
    /// &#10; file:  The id of a form file input element.
...
OneDrive.createSaveButton = function(options) {
    ...
    /// file name will either be inferred from the URL (e.g., http://mydomain.com/file_name.ext - file name = file_name.ext )or 
    /// from the name attribute of the input element.
    /// &#10; file:  Required. The id of a form file input element.
    /// &#10; success:  Optional. A callback function invoked after the request has succeeded. Arguments: none.
    /// &#10; progress: Optional. A callback function that is periodically invoked during the file upload. Arguments: float with 
    /// value ranging from 0.0 to 100.0 representing the percentage of the file upload operation that has been completed. Will be
    /// called at least once and the final call will be passed 100 indicating completion.
    ...
    executeOpenOperation: function () {
        ...
        internalApp.fileDialog(fileDialogProperties).then(
            ...
            function (fileDialogResponse) {
                // Filter API response, and pass the file[] to the caller's success callback.
                var linkType = options[ONEDRIVE_PARAM_LINKTYPE],
                    files = [],
                    responseFiles = fileDialogResponse.data || [];

                for (var i = 0; i < responseFiles.length; i++) {
                    var responseFile = responseFiles[i];
                    files[i] = {
                        fileName: responseFile.name,
                        link: (linkType === ONEDRIVE_PARAM_LINKTYPE_WEBVIEW) ? responseFile.link : responseFile.source,
                        linkType: linkType,
                        size: responseFile.size,
                        thumbnails: responseFile.images && responseFile.images.map(function (image)
                        {
                            return image.source;
                        }) || []
                    };
        ...
        internalApp.fileDialog(fileDialogProperties).then(
            ...
            function (fileDialogResponse) {
                var formUploadProperties = {
                    path: fileDialogResponse.data[0].id,
                    element: file,
...
var WL_AUTH_SERVER = "auth_server",
...
var APIRequest = function (properties) {
    ...
    request._url = isPathFullUrl(path) ?
        path :
        getApiServiceUrl() + (path[0] === "/" ? path.substring(1) : path);
 ...
 * Is of the form PERMISSION_TYPE.ACCESS_LEVEL:PICKER_TYPE|SELECTION_TYPE|LINK_TYPE:RESOURCE_ID
 * Where RESOURCE_ID is the id of the item that the user has granted the application
 * access to.
 */
var scopeResponsePattern = new RegExp("^\\w+\\.\\w+:\\w+[\\|\\w+]+:([\\d\\w]+\\!\\d+)$");
...
AuthSession.prototype = {
    ...
    getStatus: function () {
            ...
            if (expiresIn <= 10) {
                ...
                for (var i = 0; i < scopes.length; i++) {
                    ...
                    if (result)
                    {
                        ...
                        resourceId = result[1].split('_').join('.');
                    }
                    else {
        ...
        return { status: status, session: session, resource_id : resourceId };
    },
...
function getApiServiceUrl() {
    return wl_app[WL_APISERVICE_URI];
}
...
(function () { 
    ...
    FilePickerOperation.prototype = {
        ...
        _normalizeResp: function() {
            var op = this,
                isOpenPicker = (op._props[FILEDIALOG_PARAM_MODE] === FILEDIALOG_PARAM_MODE_OPEN),
...
wl_app._locale = "en";
        ...
        wl_app[API_X_HTTP_LIVE_LIBRARY] = "Web/DEVICE_" + trimVersionBuildNumber("5.5.8508.3005");
        ...
        dfSettings[WL_APISERVICE_URI] = "https://df.apis.live.net/v5.0/";
        ...
        var currentSettings = {};
        currentSettings[WL_AUTH_SERVER] = "login.live-int.com";
        currentSettings[WL_APISERVICE_URI] = "https://current.apis.live-tst.net/v5.0/";
        currentSettings[WL_SKYDRIVE_URI] = "https://onedrive.live-int.com/";
        currentSettings[WL_SDK_ROOT] = "//current-js.live-int.net/v5.0/";

        var bvtSettings = {};
        bvtSettings[WL_AUTH_SERVER] = "login.live-int.com";
        bvtSettings[WL_APISERVICE_URI] = "https://bvt.apis.live-tst.net/v5.0/";
        bvtSettings[WL_SKYDRIVE_URI] = "https://onedrive-bvt.live-int.com/";
